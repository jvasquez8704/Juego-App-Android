...
import com.example.javasquez.hilos_persistencia_sonido.R;
...
public class Partida extends ImageView{
    ...
    public Partida(Context contexto, int nivel_dificultad){
        ...
        pantalla.getSize(maneja_coord); // determinar el tamaño de la pantalla

        tam_pantX=maneja_coord.x;

        tam_pantY=maneja_coord.y;

// Construir un layout programaticamente
...
//Lo mismo para la pelota
    ...
    public boolean toque(int x, int y){
        ...
        if(y<tam_pantY/3) return false;//invalida en primer tercio de la pantalla
    ...
    public boolean movimientoBola(){//este medodo se debe guardar en el hilo, para que la pantalla no se quede bloqueada
